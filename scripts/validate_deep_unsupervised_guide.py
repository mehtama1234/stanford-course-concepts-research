...
GUIDE = ROOT / "analysis/deep-unsupervised/lecture-guide.json"
APPROACHES = ROOT / "analysis/deep-unsupervised/lecture-approaches.json"
TRANSCRIPT_INDEX = ROOT / "raw-material/youtube/transcript-index.json"
SITE_PAGE = ROOT / "site/deep-unsupervised.html"
COURSE = "berkeley-cs294-158-deep-unsupervised-learning-spring-2024"
...
def main() -> int:
    ...
    if not isinstance(guide, list):
        errors.append("deep unsupervised lecture guide must be a list")
        guide = []
    if not isinstance(approaches, list):
        errors.append("deep unsupervised lecture approaches must be a list")
        approaches = []
    ...
    if available != expected:
        errors.append(f"expected transcript index to contain deep unsupervised lectures 1-14, got {sorted(available)}")
    ...
    for i, lecture in enumerate(guide):
        ...
        if lecture["number"] not in expected:
            errors.append(f"unexpected deep unsupervised lecture number: {lecture['number']}")
        ...
        for approach in lecture_approaches:
            ...
            if words(approach.get("how_it_works", "")) < 40:
    ...
    if SITE_PAGE.exists():
        ...
        errors.append("site/deep-unsupervised.html missing; run scripts/build_site.py")

    if errors:
        print("\n".join(errors), file=sys.stderr)
        return 1
    print("validated Deep Unsupervised Learning guide: 14 transcript-backed lecture entries")
    return 0
