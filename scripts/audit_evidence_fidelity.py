...
DISCARDED = ROOT / "analysis/evidence/evidence-discarded.json"
...
def main() -> int:
    ...
    discarded = json.loads(DISCARDED.read_text(encoding="utf-8")) if DISCARDED.exists() else []
    ...
    lines = [
        ...
        f"- Discarded generated transcript-cue records: {len(discarded)}",
    ...
    lines.extend(["", "## Discarded Records", ""])
    for record in discarded[:120]:
        concept = ", ".join(record.get("supports_concepts", []))
        lines.append(f"- {record['id']} ({concept}) — {record.get('discard_reason', 'discarded after manual review')}")

    REPORT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(
        f"audited {len(records)} published evidence records, {len(review_queue)} queued records, and {len(discarded)} discarded records; "
