...
GUIDE = ROOT / "analysis/diffusion/lecture-guide.json"
APPROACHES = ROOT / "analysis/diffusion/lecture-approaches.json"
TRANSCRIPT_INDEX = ROOT / "raw-material/youtube/transcript-index.json"
SITE_PAGE = ROOT / "site/diffusion.html"
COURSE = "stanford-cme296-diffusion-large-vision-models-spring-2026"
...
def main() -> int:
    ...
    if not isinstance(guide, list):
        errors.append("diffusion lecture guide must be a list")
        guide = []
    if not isinstance(approaches, list):
        errors.append("diffusion lecture approaches must be a list")
        approaches = []
    ...
    if available != expected:
        errors.append(f"expected transcript index to contain diffusion lectures 1-8, got {sorted(available)}")
    ...
    for i, lecture in enumerate(guide):
        ...
        if lecture["number"] not in expected:
            errors.append(f"unexpected diffusion lecture number: {lecture['number']}")
    ...
    if seen != expected:
        errors.append(f"diffusion guide must contain exactly lectures 1-8, got {sorted(seen)}")
    ...
    if SITE_PAGE.exists():
        ...
        errors.append("site/diffusion.html missing; run scripts/build_site.py")

    if errors:
        print("\n".join(errors), file=sys.stderr)
        return 1
    print("validated diffusion lecture guide: 8 transcript-backed lecture entries")
    return 0
